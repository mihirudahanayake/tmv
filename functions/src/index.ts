...
export const onMailDocCreated = onDocumentCreated(
  'mail/{docId}',
  async (event) => {
    try {
...
export const onWhatsAppDocCreated = onDocumentCreated(
  'whatsapp/{docId}',
  async (event) => {
    try {
      const { to, message } = event.data.data();
...
export const onWorkCreated = onDocumentCreated(
  'works/{workId}',
  async (event) => {
    const data = event.data.data();
    ...
    const notificationPromises = assignedUsers.map(async (userId: string) => {
